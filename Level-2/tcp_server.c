...
setup_tcp_server_communication(){
    ...
    struct sockaddr_in6 server_addr, /*structure to store the server and client info*/
                       client_addr;

    /*step 2: tcp master socket creation*/
    if ((master_sock_tcp_fd = socket(AF_INET6, SOCK_STREAM, IPPROTO_TCP )) == -1)
    {
    ...
    server_addr.sin6_family = AF_INET6;/*This socket will process ipv6 network packets*/
    server_addr.sin6_port = SERVER_PORT;/*Server will process any data arriving on port no 2000*/
   ...
   /* activate dual stack configuration */
	...
	sizeof(option)
     ...
     if( r==-1 ){
	printf("failed to activate socket options");
	exit(1);
     }
    /* Bind the server. Binding means, we are telling kernel(OS) that any data 
     * you recieve with dest ip address = IPv6 address of this machine, and tcp port no = 2000, pls send that data to this process
    ...
    if (bind(master_sock_tcp_fd, (struct sockaddr *)&server_addr, sizeof(struct sockaddr_in6)) == -1)
    {
    ...
    while(1){
		...
		inet_ntop(AF_INET6, &(client_addr.sin6_addr), str, INET_ADDRSTRLEN);
