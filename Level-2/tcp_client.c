...
setup_tcp_communication(){
    ...
    dest.sin6_port = DEST_PORT;
    struct hostent *host = (struct hostent *)gethostbyname2(SERVER_IP_ADDRESS, AF_INET6);
    dest.sin6_addr = *((struct in6_addr *)host->h_addr_list[0]);
    ...
    connect(sockfd, (struct sockaddr *)&dest,sizeof(struct sockaddr_in6));
